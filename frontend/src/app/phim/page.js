...
// Utility to calculate end date (start date + 30 days)
...
export default function DanhSachPhim() {
  ...
  // Store form state before image upload to prevent data loss
  const [tempForm, setTempForm] = useState(defaultPhim);
  const [tempSuatChieuList, setTempSuatChieuList] = useState([defaultSuatChieu]);

  // Automatically set NgayKetThuc to NgayKhoiChieu + 30 days
  useEffect(() => {
    if (form.NgayKhoiChieu) {
      setForm((prev) => ({
        ...prev,
        NgayKetThuc: calculateEndDate(form.NgayKhoiChieu),
      }));
    } else {
      setForm((prev) => ({
        ...prev,
  ...
  const handleAddSuatChieu = () => {
    ...
    setTempSuatChieuList([...tempSuatChieuList, defaultSuatChieu]);
  };
  ...
  const handleSuatChieuChange = (index, field, value) => {
    ...
    setTempSuatChieuList(updatedList);
  };
  ...
  const confirmRemoveSuatChieu = () => {
    if (deleteIndex !== null) {
      const updatedList = suatChieuList.filter((_, i) => i !== deleteIndex);
      setSuatChieuList(updatedList);
      setTempSuatChieuList(updatedList);
      setDeleteIndex(null);
    }
  ...
  const handleEdit = async (phim) => {
    try {
      ...
      setForm(newForm);
      setTempForm(newForm);
      setSuatChieuList(newSuatChieuList);
      setTempSuatChieuList(newSuatChieuList);
      setDialogOpen(true);
    } catch (err) {
  ...
  const handleAdd = () => {
    setForm(defaultPhim);
    setTempForm(defaultPhim);
    setSuatChieuList([defaultSuatChieu]);
    setTempSuatChieuList([defaultSuatChieu]);
    setDialogOpen(true);
  };

  // Save form state before image upload
  const handleImageUploadStart = useCallback(() => {
    setTempForm({ ...form });
    setTempSuatChieuList([...suatChieuList]);
  }, [form, suatChieuList]);

  // Restore form state after image upload
  const handleImageUploadComplete = useCallback(
    (url) => {
      setForm((prev) => ({ ...tempForm, img: url }));
      setSuatChieuList([...tempSuatChieuList]);
    },
    [tempForm, tempSuatChieuList]
  );

  // Handle image removal
  const handleImageRemove = useCallback(() => {
    setForm((prev) => ({ ...tempForm, img: "" }));
    setSuatChieuList([...tempSuatChieuList]);
  }, [tempForm, tempSuatChieuList]);
  ...
  return (
    <div className="flex h-full flex-col md:flex-row gap-6  pr-0 w-full">
            ...
            <div className="flex gap-2">
              ...
              <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                <DialogTrigger asChild>
                  <Button className="bg-blue-600 text-white" onClick={handleAdd}>
                    THÊM PHIM
                  </Button>
                </DialogTrigger>
